...
void SpriteEditorView::connectSignals()
{
    ...
    connect(ui->flipButton, &QPushButton::clicked, this, &SpriteEditorView::onFlipButtonClicked);
...
void SpriteEditorView::onFlipButtonClicked() {
    if (m_controller) {
        m_controller->flipHorizontal();  // 좌우 반전
        updateCanvasDisplay();           // 화면 갱신
    }
}
